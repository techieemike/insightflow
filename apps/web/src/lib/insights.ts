...
export async function generateInsights(datasetId: string, rows: any[], columns: string[], fileType?: string, content?: string) {
  try {
    let insights: any;
    if (fileType === 'document' && content) {
      if (genAI) {
        try {
          insights = await generateDocumentSummary(content);
        } catch (err) {
          console.error('Gemini document summary failed, using rule-based:', err);
          insights = generateRuleBasedDocumentSummary(content);
        }
      } else {
        insights = generateRuleBasedDocumentSummary(content);
      }
      ...
      if (genAI) {
        try {
          insights = await generateWithAI(rows, columns, stats);
        } catch (err) {
          console.error('Gemini AI failed, using rule-based:', err);
          insights = generateRuleBased(rows, columns, stats);
        }
      } else {
        insights = generateRuleBased(rows, columns, stats);
      }
...
async function generateDocumentSummary(content: string): Promise<any> {
  ...
  const prompt = [
    ...
    '"keyPoints" (array of 3-6 concise bullet points),',
    '"wordCount" (number),',
  ...
  parsed.estimatedReadMinutes = Math.max(1, Math.ceil(words / 250));
  return parsed;
}

function generateRuleBasedDocumentSummary(content: string): any {
  const words = content.split(/\s+/).filter(Boolean).length;
  const sentences = content.split(/[.!?]+/).filter(Boolean).length;
  const chars = content.length;
  const uniqueWords = new Set(content.toLowerCase().split(/\s+/).filter(Boolean)).size;

  const firstSentence = sentences > 0 ? content.split(/[.!?]+/)[0].trim() + '.' : '';

  return {
    summary: firstSentence
      ? `The document begins: "${firstSentence}" It contains ${words} words across ${sentences} sentences with approximately ${chars} characters.`
      : `This document contains ${words} words across ${sentences} sentences.`,
    keyPoints: [
      `Total word count: ${words.toLocaleString()}`,
      `Unique words: ${uniqueWords.toLocaleString()}`,
      `Estimated reading time: ${Math.max(1, Math.ceil(words / 250))} minutes`,
      `Character count: ${chars.toLocaleString()}`,
    ],
    wordCount: words,
    estimatedReadMinutes: Math.max(1, Math.ceil(words / 250)),
  };
}

export function computeStats(rows: any[], columns: string[]) {
...
async function generateWithAI(rows: any[], columns: string[], stats: any) {
...
function generateRuleBased(rows: any[], columns: string[], stats: any) {
