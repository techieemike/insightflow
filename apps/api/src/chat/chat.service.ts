...
export class ChatService {
  ...
  constructor(private prisma: PrismaService, private config: ConfigService) {
    ...
    if (key && key.startsWith('AIzaSy')) this.genAI = new GoogleGenerativeAI(key);
  }
  ...
  async chat(datasetId: string, question: string) {
    ...
    const context = scored.slice(0, 30).map(r => r.data);
    ...
    try {
      if (this.genAI) {
        answer = await this.askGemini(dataset, question, context);
      } else {
        answer = this.conversationalFallback(dataset, question, context);
      }
    } catch (err) {
  ...
  private async askGemini(dataset: any, question: string, context: any[]) {
    ...
    const prompt = [
      ...
      'Answer conversationally. If asked a greeting, greet back. If asked about data, analyze it. Be concise.',
      '',
  ...
  private conversationalFallback(dataset: any, question: string, context: any[]) {
    ...
    if (/what can you (do|help|tell)|how (do|can) you (work|help)|capabilities|features|help/.test(q))
      return this.capabilitiesMessage(cols, dataset);
    ...
    if (/summary|summarize|overview|tell me about|describe/.test(q))
      return `This dataset has ${dataset.totalRecords} records across ${cols.length} columns: ${cols.join(', ')}. Quality score: ${dataset.qualityScore || 'N/A'}/100.${context.length > 0 ? ' ' + this.summarizeContext(context.slice(0, 3), cols) : ''}`;

    if (/best|top|highest|most|leading/.test(q))
      return this.topPerformer(dataset, context, cols);

    if (/worst|lowest|least|bottom|weakest/.test(q))
      return this.lowestPerformer(dataset, context, cols);
    ...
    if (/compare|versus|vs|difference/.test(q))
      return this.compareCategories(dataset, context, cols, q);
    ...
    if (/list|show|what (are|is)|which/.test(q))
      return this.whichQuestion(dataset, context, cols, q);
    ...
    if (sample.length > 0)
      return `Based on the data I have, ${this.summarizeContext(sample, cols)} Is there something specific you'd like to drill into?`;

    const dataSummary = this.summarizeContext(
  ...
  private capabilitiesMessage(cols: string[], dataset: any) {
  ...
  private whichQuestion(dataset: any, context: any[], cols: string[], q: string) {
    const catCol = cols.find(c => context.some(r => isNaN(Number(r[c]))));
    const numCol = cols.find(c => context.some(r => !isNaN(Number(r[c]))));
    if (!catCol || !numCol) return this.summarizeContext(context.slice(0, 3), cols);
    const grouped = this.buildGrouped(context, catCol, numCol);
    if (grouped.length === 0) return `No data found for ${numCol} by ${catCol}.`;
    return `Here's what I found:\n${grouped.slice(0, 5).map((g, i) => `${i + 1}. ${g.name}: ${this.fmt(g.sum)}`).join('\n')}`;
  }

  private compareCategories(dataset: any, context: any[], cols: string[], q: string) {
    const catCol = cols.find(c => context.some(r => isNaN(Number(r[c]))));
    const numCol = cols.find(c => context.some(r => !isNaN(Number(r[c]))));
    if (!catCol || !numCol) return 'Not enough data to compare.';
    const grouped = this.buildGrouped(context, catCol, numCol);
    if (grouped.length < 2) return 'Need at least two categories to compare.';
    return `Comparing ${numCol} across ${catCol}:\n${grouped.map(g => `• ${g.name}: ${this.fmt(g.sum)} (avg ${this.fmt(g.avg)})`).join('\n')}`;
  }

  private topPerformer(dataset: any, context: any[], cols: string[]) {
    const catCol = cols.find(c => context.some(r => isNaN(Number(r[c]))));
    const numCol = cols.find(c => context.some(r => !isNaN(Number(r[c]))));
    ...
    let msg = `Top ${catCol} by ${numCol}: ${top.name} (${this.fmt(top.sum)}, ${pct}% of total).`;
    if (grouped.length > 1) msg += ` Next: ${grouped.slice(1, 3).map(g => `${g.name} (${this.fmt(g.sum)})`).join(', ')}.`;
    return msg;
  }

  private lowestPerformer(dataset: any, context: any[], cols: string[]) {
    const catCol = cols.find(c => context.some(r => isNaN(Number(r[c]))));
    const numCol = cols.find(c => context.some(r => !isNaN(Number(r[c]))));
    ...
    return `Lowest ${catCol} by ${numCol}: ${low.name} (${this.fmt(low.sum)}).`;
  }

  private buildGrouped(context: any[], catCol: string, numCol: string) {
  ...
  private summarizeContext(sample: any[], cols: string[]) {
    const parts: string[] = [];
    for (const col of cols) {
      ...
      if (nums.length > 0) {
        ...
        parts.push(`${col} ranges from ${this.fmt(Math.min(...nums))} to ${this.fmt(Math.max(...nums))} (total ${this.fmt(sum)})`);
      } else {
